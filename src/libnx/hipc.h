...
inline HipcStaticDescriptor hipcMakeSendStatic(const void* buffer, size_t size, u8 index) {
...
inline HipcBufferDescriptor hipcMakeBuffer(const void* buffer, size_t size, HipcBufferMode mode) {
...
inline HipcRecvListEntry hipcMakeRecvStatic(void* buffer, size_t size) {
...
inline void* hipcGetStaticAddress(const HipcStaticDescriptor* desc) {
    return (void*)(desc->address_low | ((uintptr_t)desc->address_mid << 32) | ((uintptr_t)desc->address_high << 36));
}

inline size_t hipcGetStaticSize(const HipcStaticDescriptor* desc) {
    return desc->size;
}

inline void* hipcGetBufferAddress(const HipcBufferDescriptor* desc) {
    return (void*)(desc->address_low | ((uintptr_t)desc->address_mid << 32) | ((uintptr_t)desc->address_high << 36));
}

inline size_t hipcGetBufferSize(const HipcBufferDescriptor* desc) {
    return desc->size_low | ((size_t)desc->size_high << 32);
}

inline HipcRequest hipcCalcRequestLayout(HipcMetadata meta, void* base) {
    // Copy handles
    hk::svc::Handle* copy_handles = nullptr;
    if (meta.num_copy_handles) {
        copy_handles = (hk::svc::Handle*)base;
        base = copy_handles + meta.num_copy_handles;
    }

    // Move handles
    hk::svc::Handle* move_handles = nullptr;
    if (meta.num_move_handles) {
        move_handles = (hk::svc::Handle*)base;
        base = move_handles + meta.num_move_handles;
    }

    // Send statics
    HipcStaticDescriptor* send_statics = nullptr;
    ...
    // Send buffers
    HipcBufferDescriptor* send_buffers = nullptr;
    ...
    // Recv buffers
    HipcBufferDescriptor* recv_buffers = nullptr;
    ...
    // Exch buffers
    HipcBufferDescriptor* exch_buffers = nullptr;
    ...
    // Recv list
    HipcRecvListEntry* recv_list = nullptr;
...
inline HipcResponse hipcParseResponse(void* base) {
    // Parse header
    HipcHeader hdr = {};
    __builtin_memcpy(&hdr, base, sizeof(hdr));
    base = (u8*)base + sizeof(hdr);
    ...
    // Copy handles
    response.copy_handles = (hk::svc::Handle*)base;
    base = response.copy_handles + response.num_copy_handles;

    // Move handles
    response.move_handles = (hk::svc::Handle*)base;
