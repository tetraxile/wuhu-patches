...
static inline Waiter waiterForHandle(hk::svc::Handle h) {
    Waiter wait_obj;
 ...
 * @param[in] timeout Timeout (in nanoseconds).
 */
static inline hk::Result waitSingle(Waiter w, u64 timeout) {
    s32 idx;
 ...
 * @param[in] timeout Timeout (in nanoseconds).
 */
static inline hk::Result waitSingleHandle(hk::svc::Handle h, u64 timeout) {
    s32 idx;
