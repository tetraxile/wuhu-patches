...
constexpr u64 smServiceNameToU64(SmServiceName name) {
    u64 ret = 0;
...
constexpr SmServiceName smServiceNameFromU64(u64 name) {
    SmServiceName ret = { 0 };
 ...
 */
constexpr bool smServiceNamesAreEqual(SmServiceName a, SmServiceName b) {
 ...
 * @return Result code.
 */
hk::Result smGetServiceWrapper(Service* service_out, SmServiceName name);
 ...
 * @return Result code.
 */
hk::Result smGetServiceOriginal(hk::svc::Handle* handle_out, SmServiceName name);
 ...
 * @return Result code.
 */
inline hk::Result smGetService(Service* service_out, const char* name) {
 ...
 */
hk::svc::Handle smGetServiceOverride(SmServiceName name);
 ...
 * @return Result code.
 */
hk::Result smRegisterService(hk::svc::Handle* handle_out, SmServiceName name, bool is_light, s32 max_sessions);

/// Same as \ref smRegisterService, but always using cmif serialization.
hk::Result smRegisterServiceCmif(hk::svc::Handle* handle_out, SmServiceName name, bool is_light, s32 max_sessions);

/// Same as \ref smRegisterService, but always using tipc serialization.
hk::Result smRegisterServiceTipc(hk::svc::Handle* handle_out, SmServiceName name, bool is_light, s32 max_sessions);
 ...
 * @return Result code.
 */
hk::Result smUnregisterService(SmServiceName name);

/// Same as \ref smUnregisterService, but always using cmif serialization.
hk::Result smUnregisterServiceCmif(SmServiceName name);

/// Same as \ref smUnregisterService, but always using tipc serialization.
hk::Result smUnregisterServiceTipc(SmServiceName name);
 ...
 */
void smAddOverrideHandle(SmServiceName name, hk::svc::Handle handle);
