...
typedef struct CmifResponse {
    void* data;
    u32* objects;
    hk::svc::Handle* copy_handles;
    hk::svc::Handle* move_handles;
} CmifResponse;

inline void* cmifGetAlignedDataStart(u32* data_words, void* base) {
    intptr_t data_start = ((u8*)data_words - (u8*)base + 15) & ~15;
    return (u8*)base + data_start;
}

inline CmifRequest cmifMakeRequest(void* base, CmifRequestFormat fmt) {
...
inline void* cmifMakeControlRequest(void* base, u32 request_id, u32 size) {
...
inline void cmifMakeCloseRequest(void* base, u32 object_id) {
    if (object_id) {
        HipcRequest hipc = hipcMakeRequestInline(base,
...
constexpr void cmifRequestInBuffer(CmifRequest* req, const void* buffer, size_t size, HipcBufferMode mode) {
    *req->hipc.send_buffers++ = hipcMakeBuffer(buffer, size, mode);
}

constexpr void cmifRequestOutBuffer(CmifRequest* req, void* buffer, size_t size, HipcBufferMode mode) {
    *req->hipc.recv_buffers++ = hipcMakeBuffer(buffer, size, mode);
}

constexpr void cmifRequestInOutBuffer(CmifRequest* req, void* buffer, size_t size, HipcBufferMode mode) {
    *req->hipc.exch_buffers++ = hipcMakeBuffer(buffer, size, mode);
}

constexpr void cmifRequestInPointer(CmifRequest* req, const void* buffer, size_t size) {
    *req->hipc.send_statics++ = hipcMakeSendStatic(buffer, size, req->cur_in_ptr_id++);
    req->server_pointer_size -= size;
}

constexpr void cmifRequestOutFixedPointer(CmifRequest* req, void* buffer, size_t size) {
    *req->hipc.recv_list++ = hipcMakeRecvStatic(buffer, size);
    req->server_pointer_size -= size;
}

constexpr void cmifRequestOutPointer(CmifRequest* req, void* buffer, size_t size) {
    cmifRequestOutFixedPointer(req, buffer, size);
    *req->out_pointer_sizes++ = size;
}

constexpr void cmifRequestInAutoBuffer(CmifRequest* req, const void* buffer, size_t size, HipcBufferMode mode) {
    if (req->server_pointer_size && size <= req->server_pointer_size) {
        cmifRequestInPointer(req, buffer, size);
        cmifRequestInBuffer(req, nullptr, 0, mode);
    } else {
        cmifRequestInPointer(req, nullptr, 0);
        cmifRequestInBuffer(req, buffer, size, mode);
    }
}

constexpr void cmifRequestOutAutoBuffer(CmifRequest* req, void* buffer, size_t size, HipcBufferMode mode) {
    if (req->server_pointer_size && size <= req->server_pointer_size) {
        cmifRequestOutPointer(req, buffer, size);
        cmifRequestOutBuffer(req, nullptr, 0, mode);
    } else {
        cmifRequestOutPointer(req, nullptr, 0);
        cmifRequestOutBuffer(req, buffer, size, mode);
    }
}

constexpr void cmifRequestObject(CmifRequest* req, u32 object_id) {
    *req->objects++ = object_id;
}

constexpr void cmifRequestHandle(CmifRequest* req, hk::svc::Handle handle) {
    *req->hipc.copy_handles++ = handle;
}

inline hk::Result cmifParseResponse(CmifResponse* res, void* base, bool is_domain, u32 size) {
    HipcResponse hipc = hipcParseResponse(base);
    void* start = cmifGetAlignedDataStart(hipc.data_words, base);

    CmifOutHeader* hdr = nullptr;
    u32* objects = nullptr;
    if (is_domain) {
    ...
    HK_ASSERT(hdr->magic == CMIF_OUT_HEADER_MAGIC);
...
constexpr u32 cmifResponseGetObject(CmifResponse* res) {
    return *res->objects++;
}

constexpr hk::svc::Handle cmifResponseGetCopyHandle(CmifResponse* res) {
    return *res->copy_handles++;
}

constexpr hk::svc::Handle cmifResponseGetMoveHandle(CmifResponse* res) {
    return *res->move_handles++;
}

inline hk::Result cmifConvertCurrentObjectToDomain(hk::svc::Handle h, u32* out_object_id) {
    cmifMakeControlRequest(hk::svc::getTLSPtr(), 0, 0);
    hk::Result rc = hk::svc::SendSyncRequest(h);
    if (rc.succeeded()) {
        CmifResponse resp = {};
        rc = cmifParseResponse(&resp, hk::svc::getTLSPtr(), false, sizeof(u32));
        if (rc.succeeded() && out_object_id)
            *out_object_id = *(u32*)resp.data;
    }
    return rc;
}

inline hk::Result cmifCopyFromCurrentDomain(hk::svc::Handle h, u32 object_id, hk::svc::Handle* out_h) {
    void* raw = cmifMakeControlRequest(hk::svc::getTLSPtr(), 1, sizeof(u32));
    *(u32*)raw = object_id;
    hk::Result rc = hk::svc::SendSyncRequest(h);
    if (rc.succeeded()) {
        CmifResponse resp = {};
        rc = cmifParseResponse(&resp, hk::svc::getTLSPtr(), false, 0);
        if (rc.succeeded() && out_h)
            *out_h = resp.move_handles[0];
    }
    return rc;
}

inline hk::Result cmifCloneCurrentObject(hk::svc::Handle h, hk::svc::Handle* out_h) {
    cmifMakeControlRequest(hk::svc::getTLSPtr(), 2, 0);
    hk::Result rc = hk::svc::SendSyncRequest(h);
    if (rc.succeeded()) {
        CmifResponse resp = {};
        rc = cmifParseResponse(&resp, hk::svc::getTLSPtr(), false, 0);
...
inline hk::Result cmifQueryPointerBufferSize(hk::svc::Handle h, u16* out_size) {
    cmifMakeControlRequest(hk::svc::getTLSPtr(), 3, 0);
    hk::Result rc = hk::svc::SendSyncRequest(h);
    if (rc.succeeded()) {
        CmifResponse resp = {};
        rc = cmifParseResponse(&resp, hk::svc::getTLSPtr(), false, sizeof(u16));
        if (rc.succeeded() && out_size)
            *out_size = *(u16*)resp.data;
    }
    return rc;
}

inline hk::Result cmifCloneCurrentObjectEx(hk::svc::Handle h, u32 tag, hk::svc::Handle* out_h) {
    void* raw = cmifMakeControlRequest(hk::svc::getTLSPtr(), 4, sizeof(u32));
    *(u32*)raw = tag;
    hk::Result rc = hk::svc::SendSyncRequest(h);
    if (rc.succeeded()) {
        CmifResponse resp = {};
