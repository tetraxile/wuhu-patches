...
typedef struct SfDispatchParams {
    ...
    u32 in_num_handles;
    hk::svc::Handle in_handles[8];

    u32 out_num_objects;
    Service* out_objects;

    SfOutHandleAttrs out_handle_attrs;
    hk::svc::Handle* out_handles;
} SfDispatchParams;
 ...
 */
constexpr bool serviceIsActive(Service* s) {
    return s->session != 0;
}
 ...
 */
constexpr bool serviceIsOverride(Service* s) {
 ...
 */
constexpr bool serviceIsDomain(Service* s) {
    return serviceIsActive(s) && s->own_handle && s->object_id;
}
 ...
 */
constexpr bool serviceIsDomainSubservice(Service* s) {
    return serviceIsActive(s) && !s->own_handle && s->object_id;
}
 ...
 */
constexpr u32 serviceGetObjectId(Service* s) {
    return s->object_id;
}
 ...
 */
inline void serviceCreate(Service* s, hk::svc::Handle h) {
    s->session = h;
    s->own_handle = 1;
    s->object_id = 0;
 ...
 */
inline void serviceCreateNonDomainSubservice(Service* s, Service* parent, hk::svc::Handle h) {
    if (h != 0) {
        s->session = h;
        s->own_handle = 1;
        s->object_id = 0;
 ...
 */
constexpr void serviceCreateDomainSubservice(Service* s, Service* parent, u32 object_id) {
    if (object_id != 0) {
...
inline void serviceClose(Service* s) {
    ...
    if (s->own_handle || s->object_id) {
        cmifMakeCloseRequest(hk::svc::getTLSPtr(), s->own_handle ? 0 : s->object_id);
        hk::svc::SendSyncRequest(s->session);
        if (s->own_handle)
            hk::svc::CloseHandle(s->session);
    }
    *s = (Service) {};
}
 ...
 */
inline hk::Result serviceClone(Service* s, Service* out_s) {
#if defined(NX_SERVICE_ASSUME_NON_DOMAIN)
    if (s->object_id)
        __builtin_unreachable();
#endif
 ...
 * @param[out] out_s Output service object.
 */
inline hk::Result serviceCloneEx(Service* s, u32 tag, Service* out_s) {
#if defined(NX_SERVICE_ASSUME_NON_DOMAIN)
    if (s->object_id)
        __builtin_unreachable();
#endif
 ...
 */
inline hk::Result serviceConvertToDomain(Service* s) {
    if (!s->own_handle) {
        // For overridden services, create a clone first.
        hk::Result rc = cmifCloneCurrentObjectEx(s->session, 0, &s->session);
        if (rc.failed())
            return rc;
        s->own_handle = 1;
    }
...
constexpr void _serviceRequestFormatProcessBuffer(CmifRequestFormat* fmt, u32 attr) {
    if (!attr)
        return;
...
constexpr void _serviceRequestProcessBuffer(CmifRequest* req, const SfBuffer* buf, u32 attr) {
    if (!attr)
        return;
...
inline void* serviceMakeRequest(
    ...
    u32 num_handles, const hk::svc::Handle* handles) {
#if defined(NX_SERVICE_ASSUME_NON_DOMAIN)
    if (s->object_id)
        __builtin_unreachable();
#endif
    ...
    CmifRequest req = cmifMakeRequest(hk::svc::getTLSPtr(), fmt);
...
constexpr void _serviceResponseGetHandle(CmifResponse* res, SfOutHandleAttr type, hk::svc::Handle* out) {
    switch (type) {
    default:
...
inline hk::Result serviceParseResponse(
    Service* s, u32 out_size, void** out_data,
    u32 num_out_objects, Service* out_objects,
    const SfOutHandleAttrs out_handle_attrs, hk::svc::Handle* out_handles) {
#if defined(NX_SERVICE_ASSUME_NON_DOMAIN)
    if (s->object_id)
        __builtin_unreachable();
#endif
    ...
    hk::Result rc = cmifParseResponse(&res, hk::svc::getTLSPtr(), is_domain, out_size);
    if (rc.failed())
        return rc;
...
inline hk::Result serviceDispatchImpl(
    ...
    hk::Result rc = hk::svc::SendSyncRequest(disp.target_session == 0 ? s->session : disp.target_session);
    if (rc.succeeded()) {
        void* out = NULL;
        ...
        if (rc.succeeded() && out_data && out_data_size)
