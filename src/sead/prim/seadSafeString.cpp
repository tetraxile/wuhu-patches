...
namespace sead {
    ...
    template <typename T>
    SafeStringBase<T>& SafeStringBase<T>::operator=(const SafeStringBase<T>& other) {
    ...
    SafeStringBase<char16>::operator=(const SafeStringBase<char16>& other)
        = default;
    ...
    s32 BufferedSafeStringBase<char>::formatV(const char* fmt, std::va_list args) {
        return vsnprintf(getMutableStringTop_(), mBufferSize, fmt, args);
    }
